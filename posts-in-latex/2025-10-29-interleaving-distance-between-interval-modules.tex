...
Here \(\mathbf{0}\) denotes the zero vector space of course.
...
since $d_T (U, V) $  does not imply \(U\cong V\).
...
but in any case \(\alpha_s\) is the zero map.
...
\(\alpha\) is nothing more than multiplication by some scalar \(\lambda\in \mathbf{k}\), so \(\alpha_s\colon x\mapsto \lambda x\).
Such \(\lambda\) could be zero, however.
...
\end{figure}
Take the arrow \(a\to r\) and apply both 
\(\mathbf{k}^I\) and \(\mathbf{k}^J\).
We get  
We know \(\mathbf{k}^I(a\to r) = \)
By \eqref{eq:naturality-morphism-persistence-modules}
The conclusion of this case is that 
$\mathrm{Hom}(\mathbf{k}^I, \mathbf{k}^J)\cong \mathbf{0} $,
even if \(I\cap J\neq \varnothing\).

\item[\textbf{Case 2.}] Suppose \(c\leq a\), meaning that \(J\) \textit{starts} before \(I\). Now we consider two more cases.
\item[\textbf{Case 2.1.}] Suppose \(b <d\).
We also get $\mathrm{Hom}(\mathbf{k}^I, \mathbf{k}^J)\cong \mathbf{0} $.
the problem was that \(J\) \textit{died} after \(I\).
...
\({\mathcal{I}}=\mathbf{k}^I\) and \({\mathcal{I}}=\mathbf{k}^J\)
exists, that is,
for which we can find maps 
$ \phi\colon {\mathcal{I}}\to {\mathcal{I}}[t]$ and $ \psi\colon {\mathcal{I}}\to {\mathcal{I}}[t]   $ 
that make the diagrams
$$
\begin{tikzcd}
{\mathcal{I}} \arrow[rd, "\phi"'] \arrow[rr, "\mathbb{1}^{2t}_{{\mathcal{I}}}"] &                                 & {{\mathcal{I}}[2t]} & {\mathcal{I}} \arrow[rd, "\psi"'] \arrow[rr, "\mathbb{1}^{2t}_{{\mathcal{I}}}"] &                                 & {{\mathcal{I}}[2t]} \\
                                                      & {{\mathcal{I}}[t]} \arrow[ru, "{\psi[t]}"'] &         &                                                       & {{\mathcal{I}}[t]} \arrow[ru, "{\phi[t]}"'] &        
\end{tikzcd}
$$
commute.
