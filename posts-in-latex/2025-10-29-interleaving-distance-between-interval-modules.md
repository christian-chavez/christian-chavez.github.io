...
::: example
**Example 1** (Important). Let $I\subseteq \mathbb{R}$ be an interval.
...
\mathbf{0}&\text{ else }
\end{cases}$$ on objects and morphisms, respectively. Here $\mathbf{0}$
...
Interval modules are the building blocks of persistence modules. In
...
-   Since $V$ is a functor, we have the so-called **composition law**:
    $$v_t^s\circ v_s^r = v_t^r\quad\text{ whenever } r\leq s\leq t$$ and
    $V(t\to t) = v_t^t = \mathrm{Id}_{V_t}$.

Thus, a persistence module $V$ is a family of vector spaces
$(V_t)_{t\in \mathbb{R}}$ together with a family
$(v_t^s \colon V_s\to V_t\mid s\leq t)_{s,t\in\mathbb{R}}$ of
$\mathbf{k}$-linear maps that satisfy the composition law.
...
$$\label{eq:naturality-morphism-persistence-modules}
![diagram 1](/assets/media/post-interleaving/diagram_01.svg)$$
commutes for all $s\leq t$, i.e.,
$v_t^s \circ \alpha_s=\alpha_t\circ u_t^s$.

[Explain composition with a diagram.]{style="color: red"}
...
$$\alpha_r = \mathbf{k}^{[0,1]}(x)\to\mathbf{k}^{[0,2]}(x)$$ So
...
![diagram 2](/assets/media/post-interleaving/diagram_02.svg)
...
new persistence module $U[t]$ by $$% U[t](r)=(U[t])_r = U_{t+r}
% \quad\text{and}\quad 
% U[t](r\to s)=(u[t])_r^s = u_{t+r}^{t+s}
%
U[t](r)  = U_{r+t}
\quad\text{and}\quad 
U[t](r\to s) = u_{r+t}^{s+t}.$$ We say that $U[t]$ is a **shifted
module**. Note that $$U[0]=U
\quad\text{and}\quad
U[t][t'] = U[t+t']$$ for any $t,t'\in \mathbb{R}$. Similarly, for any
...
$$U[t](p\to q) \;=\; U_{p+t}\xrightarrow{u_{q+t}^{p+t}} U_{q+t}.$$ Since
$\alpha=(\alpha_r )_{r\in \mathbb{R}}$ is determined by its components,
we need to find the vertical arrows that make the square

![diagram 3](/assets/media/post-interleaving/diagram_03.svg)
...
-   Observe that
    ...
    =\mathbb{1}_{U}$$ is an identity natural transformation. So, for
...
1.  Objects: for each $t\in \mathbb{R}$ we have a functor
    ...
    $T_t(\alpha) = \alpha[t] = (\alpha_{r+t})_{r\in \mathbb{R}}$ on
...
::: {#ex:shifted.interval.module .example}
...
\end{cases}
=
\mathbf{k}^{I-t}(r)$$ and similarly, for any $r\leq s$ we obtain
$$\mathbf{k}^I(r+t\to s+t) = \mathbf{k}^{I-t}(r\to s).$$ Thus
...
![diagram 4](/assets/media/post-interleaving/diagram_04.svg)
...
::: example
**Example 6**. [An example that proves the last
...
Our strategy is to find some apropriate bounds for $d_T (U, V)$. Befor
...
![image](../assets/media/morphism_interval_modules.png){width="75%"}
...
1.  Suppose $a < c$. Recall that our goal is to characterize the
    ...
    ![image](../assets/media/case-a-less-than-c.png){width="75%"}
...
If $c\leq a\leq d\leq b$, then
$\mathrm{Hom}(\mathbf{k}^I, \mathbf{k}^J)\cong \mathbf{k}$. Given
...
\end{cases}$$ where $\mu_\lambda\colon r\mapsto \lambda r$. The map
...
$$![diagram 5](/assets/media/post-interleaving/diagram_05.svg)$$
commute. By using the fact that a shift of an interval module is the
interval module of the shifted interval (Example
[4](#ex:shifted.interval.module){reference-type="ref"
reference="ex:shifted.interval.module"}), we can redraw these diagram as
$$![diagram 6](/assets/media/post-interleaving/diagram_06.svg)$$
...
![image](../assets/media/intervalAB-empty-intersection.png){width="50%"}
...
$$\label{eq:inequalities-for-non-trivial-hom}
\begin{aligned}
...
\end{aligned}$$ Now we start looking for values of $t\geq0$ for which
...
1.  Suppose $d-t < a$ and $b-t<c$. Using the fact that
    ...
    $$b-2t = b -t -t \leq b-(b-d)-t \leq d-t < a$$ whence
    $t > \frac{b-a}{2}$. Similarly, if $b-t < c$, we get
    $t> \frac{d-c}{2}$. Thus
    $t\geq \max\{\frac{b-a}{2},\frac{d-c}{2}\}$, which lead us to the
...
2.  Suppose then that $a\leq d-t$ or $c\leq b-t$. Using the assumption
    that $t\geq\max\{|c-a|,|d-b|\}$, we see that either inequality
    implies the other. Thus we have $a\leq d-t$ and $c\leq b-t$.
    Therefore both sets of inequalities in
    [\[eq:inequalities-for-non-trivial-hom\]](#eq:inequalities-for-non-trivial-hom){reference-type="eqref"
    reference="eq:inequalities-for-non-trivial-hom"} above are
    satisfied, whence
...
Let's prove that
[\[eq:upper-bounds\]](#eq:upper-bounds){reference-type="eqref"
reference="eq:upper-bounds"} is actually an equality. Denote that
